...
<?php
	...
	//	$hinhanh=$_POST['hinhanh'];
		...
		$query=mysqli_query(mysqli_connect("localhost","root","","dienthoai"),$insert);
		if($query) {
      ...
      <table>
            ...
            <tr>
            	<td>Mã DM</td><td>
                    ...
                    <?php
						$show = mysqli_query(mysqli_connect("localhost","root","","dienthoai"),"SELECT * FROM danhmuc WHERE dequi=0");
						while($show1 = mysqli_fetch_array($show))
						{
							$madm1 = $show1['madm'];	
							$tendm1 = $show1['tendm'];
							echo "<option value='".$madm1."'>".$tendm1."</option>";	
								$show2 = mysqli_query(mysqli_connect("localhost","root","","dienthoai"),"SELECT * FROM danhmuc WHERE dequi='".$madm1."'");
